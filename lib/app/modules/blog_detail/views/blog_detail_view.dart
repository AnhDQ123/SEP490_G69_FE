...
class BlogDetailView extends GetView<BlogDetailController> {
  BlogDetailView({Key? key}) : super(key: key);
    ...
    return Row(
      children: [
        ...
        Column(
          ...
          children: [
            Text(
              controller.blog.writer?.name ?? 'Ẩn danh', // Sửa ở đây
              style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
            ),
            if (controller.blog.createdAt != null) // Thêm kiểm tra null
              Text(
              ...
              children: [
                Expanded(
                  child: Row(
                    children: [
                      _buildGridImage(imageUrls[0],
                          flex: 1, height: imageHeight / 2, index: 0),
                      const SizedBox(width: 4),
                      _buildGridImage(imageUrls[1],
                          flex: 1, height: imageHeight / 2, index: 1),
                    ],
                  ),
                ),
                const SizedBox(height: 4),
                Expanded(
                  child: Row(
                    children: [
                      _buildGridImage(imageUrls[2],
                          flex: 1, height: imageHeight / 2, index: 2),
                      const SizedBox(width: 4),
                      _buildGridImage(imageUrls[3],
                          flex: 1, height: imageHeight / 2, index: 3),
                    ],
                  ),
                ),
              ],
  ...
  Widget _buildCommentSection() {
    return Obx(() {
      return Column(
        ...
        children: [
          Text(
            'Bình luận (${controller.blog.commentCount ?? 0})',
            style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
          ),
          ...
          else if (controller.isLoadingComments.value)
  ...
  Widget _buildCommentItem(Comment comment) {
    final writerName = comment.writer?.name ?? 'Ẩn danh';
    final avatarProvider = comment.writer?.avatarUrl != null
  ...
  void _postComment() {
    ...
    if (content.isNotEmpty) {
      controller.postComment(content);
