...
class ShipperRegisterView extends StatelessWidget {
  final ShipperRegisterController controller =
  Get.put(ShipperRegisterController());
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildTextField(controller.fullName,
                label: "Họ và tên", isRequired: true),
            _buildDropdownField(controller.gender,
                label: "Giới tính", items: ["Nam", "Nữ"]),
            _buildDatePicker(controller.dateOfBirth, label: "Ngày sinh"),
            // _buildTextField(controller.phone,
            //     label: "Số điện thoại", isRequired: true, isNumeric: true),
            // _buildEmailField(controller.email),
            _buildTextField(controller.idNumber,
                label: "Số CMND/Căn cước", isRequired: true, isNumeric: true),
            _buildDatePicker(controller.expiryDate,
                label: "Hạn CCCD", isFutureOnly: true),
            const SizedBox(height: 12),
            _buildImageUploader(
              label: "Ảnh mặt trước của CCCD",
              imageController: controller.idFrontImage,
              onGalleryPick: () =>
                  controller.pickImageFromGallery(controller.idFrontImage),
              onCameraPick: () =>
                  controller.pickImageFromCamera(controller.idFrontImage),
            ...
            _buildImageUploader(
              ...
              onGalleryPick: () =>
                  controller.pickImageFromGallery(controller.idBackImage),
              onCameraPick: () =>
                  controller.pickImageFromCamera(controller.idBackImage),
              onRemove: () => controller.removeImage(controller.idBackImage),
            ),
            const SizedBox(height: 12),
            // _buildTextField(controller.licenseNumber,
            //     label: "Giấy phép lái xe", isRequired: true, isNumeric: true),
            _buildDatePicker(controller.licenseExpiry,
                label: "Ngày hết hạn giấy phép lái xe", isFutureOnly: true),
            _buildImageUploader(
              label: "Ảnh mặt trước của giấy phép lái xe",
              imageController: controller.licenseFrontImage,
              onGalleryPick: () =>
                  controller.pickImageFromGallery(controller.licenseFrontImage),
              onCameraPick: () =>
                  controller.pickImageFromCamera(controller.licenseFrontImage),
              onRemove: () =>
                  controller.removeImage(controller.licenseFrontImage),
            ),
            const SizedBox(height: 12),
            _buildImageUploader(
              ...
              onGalleryPick: () =>
                  controller.pickImageFromGallery(controller.licenseBackImage),
              onCameraPick: () =>
                  controller.pickImageFromCamera(controller.licenseBackImage),
              onRemove: () =>
                  controller.removeImage(controller.licenseBackImage),
            ),
            const SizedBox(height: 12),
            _buildImageUploader(
              ...
              onGalleryPick: () =>
                  controller.pickImageFromGallery(controller.legalRecordImage),
              onCameraPick: () =>
                  controller.pickImageFromCamera(controller.legalRecordImage),
              onRemove: () =>
                  controller.removeImage(controller.legalRecordImage),
            ...
            const SizedBox(height: 12),
            _buildTextField(controller.bankInfo,
            ...
            Center(
              child: Obx(() => OutlinedButton(
                ...
                onPressed: controller.isFormValid.value && !controller.isLoading.value
                    ? () => controller.submitRegistration() // Truyền userId vào API
                    : null,
  ...
  //         },
  //         onEditingComplete: () {
  //           if (controllerValue.value.isNotEmpty &&
  //               !GetUtils.isEmail(controllerValue.value)) {
    ...
    return Column(
      ...
      children: [
        ...
        Obx(() => DropdownButtonFormField(
          value:
          controllerValue.value.isEmpty ? null : controllerValue.value,
          items: items
              .map((item) =>
        ...
        GestureDetector(
          ...
          child: Obx(() => Container(
            ...
            child: Text(
              ...
              style: TextStyle(
                fontSize: 16,
                color: controllerValue.value.isNotEmpty
                    ? Colors.black87
                    : Colors.grey,
              ),
            ),
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        double width = isPortrait
            ? screenWidth * 0.6
            : screenWidth; // LLTP nhỏ hơn, CCCD/GPLX full width
        double height = isPortrait
            ? width * 1.5
            : width * 0.6; // LLTP cao hơn (tỷ lệ 1.5:1)
