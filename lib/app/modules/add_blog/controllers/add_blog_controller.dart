...
class AddBlogController extends GetxController {
  ...
  final selectedAssets = <XFile>[].obs;  // Sử dụng XFile từ image_picker
  ...
  Future<void> pickAssets() async {
    final pickedFiles = await _picker.pickMultiImage(); // Chọn nhiều ảnh từ thư viện

    if (pickedFiles != null) {
      selectedAssets.clear();
      selectedAssets.addAll(pickedFiles);
    }
  }
  ...
  Future<void> submitBlog() async {
    ...
    try {
      ...
      // Lấy file từ ảnh đã chọn
      final files = await Future.wait(
        selectedAssets.map((asset) async => await File(asset.path)),
      );
      ...
      if (isEditing.value && editingBlog != null) {
        ...
        // Đăng bài mới
        await BlogService().addBlog(content, files, userId);
      }
